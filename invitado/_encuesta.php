<?php 	
 ...
 $response = null;
  ...
  $select_encuesta = $mysqli->query("SELECT id_encuesta FROM encuestas WHERE estado = 1 ORDER BY id_encuesta ASC LIMIT 1");
  $row_value = $select_encuesta->fetch_assoc();
  $id_encuesta = $row_value['id_encuesta'];		

  echo $id_encuesta;

  $query = $mysqli->query("SELECT * FROM encuestas WHERE id_encuesta = '$id_encuesta'");
  $row = $query->fetch_assoc();
print_r($row);
echo "sdf";
  //$respuesta = $query->query($query);
 

  $query3 = $mysqli->("SELECT * FROM tipo_pregunta");
  $respuesta3 = $query3->fetch_assoc();
...
echo $data; */
 	...
 	$query2 = $mysqli->("SELECT * FROM preguntas WHERE id_encuesta = '$id_encuesta'");
  	$respuesta2 = $query2->fetch_assoc();

  	$query3 = $mysqli->("SELECT encuestas.titulo, encuestas.descripcion, preguntas.id_pregunta, preguntas.id_encuesta, preguntas.id_tipo_pregunta 
		FROM preguntas
